...
### The Problem
The original `main.py` was one file doing everything: fetching jobs from an API, cleaning text, generating embeddings, and scoring results. Any change to any part of the system required using the same file.
...
### The Problem
After applying SRP, `main.py` still directly instantiated `OpenAIEmbeddingService`, tightly coupling the high-level logic to OpenAI. This made it impossible to test without making real API calls to OpenAI.
